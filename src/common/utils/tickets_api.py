...
class TicketsAPI:
    ...
    def send_to_spam(self, ticket_id):
        URL = f"{self.BASE_URL}/api/issues/{ticket_id}"
    ...
    def _get_open_tickets_ids(self) -> list:
        URL = f"{self.BASE_URL}/api/sortedIssues"
    ...
    def get_open_tickets_info(self) -> list:
        URL = f"{self.BASE_URL}/api/issuesGetter"
    ...
    def get_new_ticket(self) -> list:
        ...
        ATTRIBS = f"?fields=idReadable,summary,description&query=project:{config['youtrack']['lines']['to_track_from']['project']}%20 Assignee: Unassigned State: -Closed, -{{Waiting for L2}}'"
    ...
    def _get_tickets_ids_for_sla_check(self) -> list:
        URL = f"{self.BASE_URL}/api/sortedIssues"
        ATTRIBS = f"?topRoot=100&skipRoot=0&flatten=true&query=state: {config['youtrack']['lines']['to_track_from']['sla_check']} Assignee: -Unassigned&folderId=108-0&fields=tree(id,summaryTextSearchResult(highlightRanges(startOffset,endOffset)))"
        full_url = f"{URL}{ATTRIBS}"
        tickets =  requests.get(full_url, headers=self.HEADERS).json()["tree"]
